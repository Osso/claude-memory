    ...
    D: Graph<NI> + DirectedDegrees<NI> + DirectedNeighborsWithValues<NI, EV>,
{
    ...
    /// ```
    fn out_degree_partition(&self, concurrency: usize) -> Vec<Range<NI>> {
        directed_degree_partition(self, concurrency, |graph, node| graph.out_degree(node).index())
    }
}
    ...
    D: Graph<NI> + DirectedDegrees<NI> + DirectedNeighborsWithValues<NI, EV>,
{
    ...
    /// ```
    fn in_degree_partition(&self, concurrency: usize) -> Vec<Range<NI>> {
        directed_degree_partition(self, concurrency, |graph, node| graph.in_degree(node).index())
    }
}

fn directed_degree_partition<NI, D, EV, F>(
    graph: &D,
    concurrency: usize,
    degree_fn: F,
) -> Vec<Range<NI>>
where
    ...
    F: Fn(&D, NI) -> usize,
{
    let batch_size = (graph.edge_count().index() as f64 / concurrency as f64).ceil();
    greedy_node_map_partition(
        |node| degree_fn(graph, node),
        graph.node_count(),
        batch_size as usize,
        concurrency,
    )

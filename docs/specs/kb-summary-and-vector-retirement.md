...
- [x] Preserve KB PageIndex explicit build and deterministic query retrieval.
- [x] Rebuild the KB text index automatically when query or enrich detects missing or stale data.
...
- Existing KB PageIndex tests in `src/kb_search_tests.rs`, including persisted search, automatic rebuild, and stale enrich fallback.
